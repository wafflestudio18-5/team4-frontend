...
import { profile_exp } from "../../Assets";
import {
  getAnswersOfUser,
  getQuestionsOfUser,
  getTagsOfUser,
    ...
    <Card className={classes.root}>
      ...
      >
        <CardMedia
          ...
          image={user.picture ? user.picture : profile_exp}
          title={user.nickname}
        />
        <CardContent>
          ...
          {!items.length ? (
            <ListItem key={0}>
              <ListItemText
                ...
                primary={`There is no ${activity.slice(0, -1).toLowerCase()}.`}
              />
            </ListItem>
          ) : activity === "Tags" ? (
            items.map((item) => (
              <ListItem
                key={item.id}
                onClick={() =>
                  history.push(`/search?q=user:${user.id}+[${item.name}]`)
                }
              >
                <Button
                  style={{
                ...
                />
                <ListItemText
                  style={{ paddingLeft: "1rem", textAlign: "right" }}
                  primary={`${item.posts} posts`}
                />
              </ListItem>
            ))
          ) : (
            items.map((item) => (
              ...
              >
                ...
                <ListItemText
                  style={{ paddingLeft: "1rem", textAlign: "right" }}
                  primary={item.created_at.substring(0, 10)}
                />
              </ListItem>
            ))
          )}
...
const Profile = ({ user }) => {
  ...
  const getTopTags = () => {
    getTagsOfUser(user.id, "votes").then(setTopTags).catch(console.log);
  };
  return user && topPosts && topTags ? (
        ...
        <div className="user-top-posts">
          ...
          >
            <Button
              ...
              onClick={() => {
                history.push(`/search?q=user:${user.id}`);
              }}
              size="large"
              color="primary"
            >{`Top Posts`}</Button>
            <div>
              <ButtonGroup
                style={{ height: "1.5rem", margin: "0 0.5rem 0.5rem 0" }}
                size="small"
              ...
              >
                <Button
                  style={{ textTransform: "lowercase" }}
                  onClick={(e) => {
                    setSortedBy(e.target.innerHTML.toLowerCase());
                  }}
                >
                  Votes
                </Button>
