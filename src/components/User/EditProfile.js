...
const EditProfile = () => {
  ...
  useEffect(() => {
    setIsValid(
      !password || password.match(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,30}$/)
    );
  });
  useEffect(() => {
    if (save) setTimeout(setSave(""), 100);
  }, [save]);
  const saveChange = (e) => {
    e.preventDefault();
    console.log("change");
      ...
      >
        ...
        >
          ...
          >
            <img
              ...
              src={user.picture ? user.picture : profile_exp}
              alt="user"
            />
            <Button
          ...
          >
            ...
            >
              ...
              >
                Password must be <b>8~30</b> characters containing at least one{" "}
                <b>number</b>, one <b>uppercase</b> letter and one{" "}
        ...
        >
          <FormControl>
            ...
            <Input
              style={{ marginBottom: "1rem", maxLength: "150" }}
              id="title"
              value={title}
        ...
        <div>
          ...
          <Button
            variant="contained"
            onClick={() => history.push("/users/me/activity")}
          >
            Cancel
          </Button>
        </div>
